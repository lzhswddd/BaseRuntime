...
void nn::Net::regularization(NetNode<Layer*>* tree, mat_t lambda)
{
	for (NetNode<Layer*> & node : tree->child) {
		if (node.data->isparam) {
...
void nn::Net::initialize_size(NetNode<Layer*>* tree, vector<Size3>& input_size, int idx)
{
	for (vector<NetNode<Layer*>>::iterator node = tree->child.begin(); node != tree->child.end();) {
		if (node->data->type == FULLCONNECTION)
		{
			if (input_size[idx].c != 1 || input_size[idx].w != 1)
			{
				if (node->data->start) {
					...
					node = tree->child.insert(node, CreateNode(Reshape(Size3(1, input_size[idx].h*input_size[idx].w*input_size[idx].c, 1)), &(*node)));
		...
		if (!node->sibling.empty()) {
			if (!node->sibling.empty()) {
				for (int i = 0; i < (int)node->sibling.size(); ++i) {
	...
	for (const NetNode<Layer*> & layerInfo : tree->child) {
		...
		if (!layerInfo.sibling.empty()) {
			for
				...
				(int i = 0; i < (int)layerInfo.sibling.size(); ++i) {
...
void nn::Net::forward_train(NetNode<Layer*>* tree, vector<Mat> & variable, vector<vector<Mat>>& output, int idx)
{
	for (NetNode<Layer*> & layerInfo : tree->child) {
		...
		if (!layerInfo.sibling.empty()) {
			for (int i = 0; i < (int)layerInfo.sibling.size(); ++i) {
				output.push_back(output[idx]);
...
void nn::Net::back_train(NetNode<Layer*>* tree, vector<Mat>& dlayer, vector<vector<Mat>> & output, int &number, int idx)
{
	for (int index = (int)tree->child.size() - 1; index >= 0; --index) {
		if (!tree->child[index].sibling.empty()) {
			for (int i = 0; i < (int)tree->child[index].sibling.size(); ++i) {
...
vector<Size3> nn::Net::initialize(Size3 input_size)
{
	...
	Net::initialize_size(netTree, xsize, 0);
	isinit = true;
	return xsize;
}
void nn::Net::clear()
...
void nn::Net::save(string net_name) const
{
	...
	if (file.is_open() && data) {
		json info;
		Net::save_layer(netTree, &info, data);
		file << info.dump(4) << std::endl;
		file.close(); fclose(data);
	}
	else {
		THROW_INFO_STR("打开文件失败!");
	}
}
	...
	if (file.is_open() && data) {
		...
		for (size_t i = 0; i < info.size(); ++i)
		{
			...
			if ((int)parent.size() == layer->layer_index) {
				newpos->sibling.push_back(Net::CreateNode(new Node(), newpos));
				newpos->sibling[newpos->sibling.size() - 1].child.push_back(Net::CreateNode(layer, &newpos->sibling[newpos->sibling.size() - 1]));
				parent.push_back(&newpos->sibling[newpos->sibling.size() - 1]);
			}
			else {
				parent[layer->layer_index]->child.push_back(Net::CreateNode(layer, parent[layer->layer_index]));
			}
...
{
	netTree->child.push_back(Net::CreateNode(layerInfo, netTree));
}
void nn::Net::add(Layer* layerInfo, string layer_name, bool sibling)
{
	if (layer_name == "")
		if (sibling) {
			...
			(netTree->child[netTree->child.size() - 1].sibling.end() - 1)->child.push_back(Net::CreateNode(layerInfo, &(netTree->child[netTree->child.size() - 1])));
		}
		else
			netTree->child[netTree->child.size() - 1].child.push_back(Net::CreateNode(layerInfo, &(netTree->child[netTree->child.size() - 1])));
	else
		Net::insert_layer(netTree, layer_name, layerInfo, sibling);
}
void nn::Net::addLoss(LF loss_f, string layer_name)
{
	netTree->child.push_back(Net::CreateNode(Loss(loss_f, 1, layer_name), netTree));
}
void nn::Net::addActivation(Act act_f, string layer_name)
{
	netTree->child.push_back(Net::CreateNode(Activation(act_f, layer_name), netTree));
}
void nn::Net::addMaxPool(Size poolsize, int strides, string layer_name)
{
	netTree->child.push_back(Net::CreateNode(MaxPool(poolsize, strides, layer_name), netTree));
}
void nn::Net::addMaxPool(int pool_row, int pool_col, int strides, string layer_name)
{
	netTree->child.push_back(Net::CreateNode(MaxPool(Size(pool_row, pool_col), strides, layer_name), netTree));
}
void nn::Net::addAveragePool(Size poolsize, int strides, string layer_name)
{
	netTree->child.push_back(Net::CreateNode(AveragePool(poolsize, strides, layer_name), netTree));
}
void nn::Net::addAveragePool(int pool_row, int pool_col, int strides, string layer_name)
{
	netTree->child.push_back(Net::CreateNode(AveragePool(Size(pool_row, pool_col), strides, layer_name), netTree));
}
void nn::Net::addDense(int layer_size, Act act_f, string layer_name)
{
	netTree->child.push_back(Net::CreateNode(Dense(layer_size, act_f, layer_name), netTree));
}
void nn::Net::addFullConnect(int layer_size, Act act_f, string layer_name)
{
	netTree->child.push_back(Net::CreateNode(FullConnect(layer_size, act_f, layer_name), netTree));
}
void nn::Net::addConv2D(int channel, int kern_size, bool is_copy_border, string layer_name, Size strides, Point anchor, Act act_f)
{
	netTree->child.push_back(Net::CreateNode(Conv2D(channel, kern_size, is_copy_border, act_f, layer_name, strides, anchor), netTree));
}
const vector<Mat> nn::Net::operator()(const Mat & input) const
{
	return forward(input);
}
nn::Net& nn::Net::operator=(const Net &nn)
{
	if (this == &nn)
		return *this;
	if (recount != 0) {
...
std::ostream & nn::operator << (std::ostream & out, const Net & nn)
{
	Net::show_net(nn, out);
	return out;
}
